...
**Internationalization (`public/i18n.js`).** The UI supports the NYS language-access set (12 languages + English), selected via a **globe menu fixed top-left** (`#lang-widget`, built in `app.js` `setupLanguageMenu`). **All 13 languages are baked into `i18n.js`** so switching is synchronous/instant and works offline with no API key (English + Spanish are the most reviewed; other languages cover the core UI and fall back to English per missing key). `VWRi18n.t('key', vars)` looks up strings; `VWRi18n.setLang(code)` switches and calls `window.VWRonLangChange` → re-render. Static HTML uses `data-i18n` / `data-i18n-ph` / `data-i18n-title` / `data-i18n-label` (the last sets a `nys-button` `label`). Arabic/Urdu/Yiddish set `dir="rtl"`. When adding user-facing strings, add the key to at least `en` (and ideally `es`) in `i18n.js` and use `t()` in `app.js` (or `data-i18n` in HTML). A server `/api/ai/translate-ui` endpoint exists for future on-the-fly translation but the UI no longer depends on it. The conference overlay (`conference.js`) is not yet localized.
...
**Adjournment / withdrawal requests.** Attendees emit `requestAction` ({type: 'adjournment'|'withdrawal', reason}); the Hearing Officer emits `resolveRequest` ({requestId, decision}). A granted request sets the hearing to `closed` with disposition "Adjourned"/"Withdrawn". State lives on `hearing.requests` (in the snapshot); the card shows attendee buttons + own request status, the officer sees a pending-requests panel with Grant/Deny, and the supervisor table flags pending requests. Allowed types are validated server-side.

**Evidence upload.** Participants attach documents to a hearing via `POST /api/evidence/:hearingId` (raw body + metadata in query; allow-listed extensions, 25 MB cap). Files are written to `evidence/` with a persisted `evidence/index.json`; the `evidence` array is included in the state snapshot, so the card's evidence section and the supervisor "Docs" count update live on `broadcast()`. `evidence/` is gitignored.
